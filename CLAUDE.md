...
src/
  ...
  app/
    ...
    TailorGamePopup.tsx       ← The waiting-room popup (27 Sep): opens the moment executeTailor starts a full tailor (JD or outreach), a progress bar for the REAL run (lib/games/pick progressAt: 90% at the expected ~50 s, creeping to 99%; only the real result fills it — never a timer) over a canvas mini game; the instant the result or an error lands, the game loop stops and the notice shows ("Your CV has been tailored successfully! You can now go back." + "Back to my CV", which scrolls to the results; a failure shows its error). Closing (×, Escape) never cancels the run and a closed popup never reopens. Accessible dialog: aria-modal, focus moves in and back, Tab trapped, page scroll locked, reduced motion → a Play button instead of an autostart. Keys: arrows / WASD / Space (never scroll the page; Space on a focused button presses it). Touch (coarse pointers or ≤ 640 px): ◀ ▶ bottom-left, ▲ bottom-right, passive touchstart/touchend + touch-action: none; a phone's replayed mouse events are ignored. Canvas colours are the --game-* tokens. A UI test that mocks a tailor must press "Back to my CV" before touching the page (smoke dismissGame)
  ...
  components/ui/              ← Thin wrappers over the globals.css classes: Button, Card, Input, Textarea,
                                 ...
                                 scroll in one click).
  lib/
    games/                    ← The waiting-room games (framework-free, import-free except each other; tests/games.test.ts runs the physics with no DOM): core.ts (480×320 logical view, InputState — `jumpPressed` is the press edge, every jump a FULL jump so a phone tap clears the widest pit — Palette, makeRng seeded levels, runLoop fixed 60 Hz steps with a clamped long frame, backdrop + score drawing; the only canvas text is a number), employer.ts (the player: a figure in a navy suit, white shirt, amber tie; limbPose() gives two-segment arms/legs joint angles — run cycle with opposed swing and knee flex, jump tuck, flap — drawEmployer() draws them), platformer.ts (Game 1: generated level of pits ≤ MAX_GAP 96, spike strips ≥ 80 px from any landing, blocks, a platform bridging 150–180 px pits, steps; rises ≤ MAX_RISE 96 against a ~102 px jump; a fall or spike respawns at the last ground's start in 0.45 s), flyer.ts (Game 2: gravity, flap, walls with gaps 132 → 110 px whose centres move ≤ 110 px wall to wall), pick.ts (gameForTailorCount: 1st/3rd… platformer, 2nd/4th… flyer; nextTailorCount per user in localStorage `cvtailor:games:<userId>`, memory fallback)
