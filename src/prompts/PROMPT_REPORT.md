...
| **Scoring** (`ATS_SCORING_PROMPT`) | Wave 2 | analysis + the four tailored sections | JSON hits/misses/recommendations/assessment | Verdicts are **overwritten** by `reconcileAtsScore` (deterministic matcher); the model keeps only prose | Prose only |
| **Post-generation, deterministic** | After wave 2 | The finished text | `reconcileAtsScore`, `checkClaims` (figures vs CV/pool, learning skills), `normalizeExperienceOutput` (bullet markers), refusal guard | — | — |
